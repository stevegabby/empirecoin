...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 420,000 blocks, which will occur approximately every 1 years
    nSubsidy >>= (nHeight / 420000); // Empirecoin: 420,000 blocks in 9 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Empirecoin: 1 day
static const int64 nTargetSpacing = 120; // Empirecoin: 2 minute
...
{
        static const int64 BlocksTargetSpacing = 60; // Empirecoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Empirecoin: 1 day
...
{
    // Empirecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Empirecoin: increase each by adding 6 to bitcoin's value.
